...
//	HW_TS_Create(CFG_TIM_PROC_ID_ISR, &(TimerDataThroughput_Id),
//			hw_ts_SingleShot, DataThroughputTimer);
...
//                  BSP_LED_On(LED_GREEN);
		...
		case ACI_GATT_NOTIFICATION_VSEVT_CODE: {
			...
			if ((pr->Attribute_Handle
				...
				NotificationData.DataTransfered.Length =
						pr->Attribute_Value_Length;
				NotificationData.DataTransfered.pPayload =
						(pr->Attribute_Value);
				NotificationData.DataTransfered.pPayload_n =
						*((uint32_t*) &(pr->Attribute_Value[0]));
				__disable_irq();
				if (NotificationData.DataTransfered.pPayload_n
						>= (NotificationData.DataTransfered.pPayload_n_1 + 2)) {
					packet_lost +=
							((NotificationData.DataTransfered.pPayload_n
									- NotificationData.DataTransfered.pPayload_n_1)
									- 1);
				}
				__enable_irq();
				/* CRC computation */
				CRC_Result = APP_BLE_ComputeCRC8(
						(uint8_t*) (pr->Attribute_Value),
						(pr->Attribute_Value_Length) - 1);
				/* get low weight byte */
				CRC_Received =
						(uint8_t) (pr->Attribute_Value[DATA_NOTIFICATION_MAX_PACKET_SIZE
								- 1]);
				...
				NotificationData.DataTransfered.pPayload_n_1 =
						NotificationData.DataTransfered.pPayload_n;
				if (DataTransfered == 0) {
					HW_TS_Start(TimerDataThroughput_Id,
							DATA_THROUGHPUT_MEASUREMENT);
				}
